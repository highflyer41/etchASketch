	...
	public void processKeyEvent(KeyEvent e)
	{
		...
		if ( e.getID() == KeyEvent.KEY_PRESSED )
		{
			...
			if ( e.getKeyCode() == KeyEvent.VK_DOWN )
                y += 10;
            if ( e.getKeyCode() == KeyEvent.VK_LEFT ) //allows left and right movement
                x -= 10; //change to move 10 pixels at a time instead of 5
            if ( e.getKeyCode() == KeyEvent.VK_RIGHT )
                x += 10;
            if (e.getKeyCode() == KeyEvent.VK_F1) //changes color on f keys
                cur = Color.red;
